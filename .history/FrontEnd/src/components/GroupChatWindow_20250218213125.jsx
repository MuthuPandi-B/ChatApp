...
const GroupChatWindow = ({ selectedGroup }) => {
  ...
  return (
    <div className="group-chat-window flex flex-1 flex-colbg h-full">
      <div className="messages flex-1 overflow-auto p-4">
        {messages.map((msg, index) => (
          <div key={index} className={`message ${msg.sender === userId? 'items-end' : 'items-start'}`}>
            <span>{msg.sender === userId ? 'You' : msg.sender.username}</span>
            <p>{msg.message}</p>
          </div>
        ))}
