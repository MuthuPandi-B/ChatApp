...
//       setError(error.response?.data?.message || "Error creating/updating group, please try again later.");
//     }
//   };
...
//           <div className={`w-full md:w-1/3 ${selectedGroup && windowWidth >= 768 ? 'hidden md:block' : ''} border-r border-gray-300 overflow-y-auto`}>
//             <GroupChatList
...
//           <div className="modal-content bg-white p-6 rounded-lg w-full md:w-1/3 "role="dialog" aria-labelledby="modal-title">
...
const GroupChatPage = () => {
  ...
  useEffect(() => {
    const handleResize = () =>
      setUiState((prev) => ({ ...prev, windowWidth: window.innerWidth }));
  ...
  useEffect(() => {
    if (!uiState.showCreateModal) return;
  ...
  const resetGroupForm = () => {
    setGroupForm({ name: "", members: [], editGroupData: null });
    setUiState((prev) => ({ ...prev, showCreateModal: false, error: "" }));
  };

  const handleCreateOrUpdateGroup = async (e) => {
    e.preventDefault();
    const { name, members, editGroupData } = groupForm;

    if (!name || !members.length) {
      setUiState((prev) => ({ ...prev, error: "Please fill all fields" }));
      return;
    }
    ...
    try {
      ...
      if (uiState.isEditing && editGroupData) {
        await Api.put(`/api/groups/${editGroupData._id}`, { name, members });
      } else {
        await Api.post("api/groups/create", { name, members });
      }
      ...
      setUiState((prev) => ({
        ...prev,
        error: error.response?.data?.message || "Error updating group.",
      }));
    }
  };
  ...
  const handleEditGroup = (group) => {
    ...
    setUiState((prev) => ({ ...prev, isEditing: true, showCreateModal: true }));
  };

  const handleMemberSelection = useCallback((userId) => {
    setGroupForm((prev) => ({
      ...prev,
      members: prev.members.includes(userId)
        ? prev.members.filter((member) => member !== userId)
        : [...prev.members, userId],
    }));
  }, []);
  ...
  return (
    <div className="group-chat-page flex flex-col h-full overflow-hidden">
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        {!selectedGroup || uiState.windowWidth >= 768 ? (
          <div className="w-full md:w-1/3 border-r border-gray-300 overflow-y-auto">
            <GroupChatList onSelectGroup={handleSelectGroup} onEditGroup={handleEditGroup} />
            <button
      ...
      {uiState.showCreateModal && (
        <div className="modal fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center">
          <div className="modal-content bg-white p-6 rounded-lg w-full md:w-1/3">
            <h2 className="text-xl font-semibold mb-4">
              {uiState.isEditing ? "Edit Group" : "Create New Group"}
            </h2>
            <form onSubmit={handleCreateOrUpdateGroup}>
                    ...
                    .map((user) => (
                      <div key={user._id} className="flex items-center">
                        <input
                          ...
                          onChange={() => handleMemberSelection(user._id)}
                        />
                        <label htmlFor={`user-${user._id}`} className="ml-2">
                          {user.username}
                        </label>
                      </div>
                    ))}
              ...
              {uiState.error && <p className="text-red-500 text-sm">{uiState.error}</p>}
              ...
              <div className="flex justify-end mt-4">
                ...
                <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded">
                  {uiState.isEditing ? "Update Group" : "Create Group"}
                </button>
              </div>
            </form>
          </div>
