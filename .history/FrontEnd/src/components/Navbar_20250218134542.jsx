...
const Navbar = ({ token ,userName,setUserName, setToken}) => {
  ...
  const firstLetter =userName ? userName.charAt(0).toUpperCase(): ''; // Get first letter of the username
  ...
  const handleLogout = () => {
    ...
    setUserName(""); 
    setToken(''); // Reset userName in state after logout
   navi/ Redirect to login
  };
