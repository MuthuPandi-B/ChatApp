    ...
    <div className="group-chat-window flex-1 p-4 bg-gray-100 flex flex-col h-full">
      <div className="messages overflow-y-auto mb-4 flex-1 max-h-[calc(100vh-160px)]">
        {messages.map((msg, index) => (
          <div key={index} className={`message flex flex-col mb-2 ${msg.sender._id === userId? 'items-end' : 'items-start'}`}>
            <span className='text-sm text-gray-500'>{msg.sender._id === userId ? 'You' : msg.sender.username}</span>
            <p   className={`p-2 rounded-lg max-w-xs break-words ${msg.sender === userId
